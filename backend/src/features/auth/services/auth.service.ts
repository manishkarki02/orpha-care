...
export const verifyUser = async (userEmail: string, token: string) => {
	...
	const cachedToken = await consumeCachedToken("verify-email", userEmail, token);
	if (!cachedToken) {
...
export const resetPassword = async (userEmail: string, token: string, newPassword: string) => {
	...
	const cachedToken = await consumeCachedToken("reset-password", userEmail, token);
	if (!cachedToken) {
...
export const signInUser = async (body: LoginRequestSchema["body"]) => {
	...
	if (!user.isVerified) {
		throw new AuthenticationError("User is not verified. Please verify your email.");
	}

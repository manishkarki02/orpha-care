...
A child-welfare platform (Nepal context) with four domains: **missing-child reports, adoption, donations, and volunteers**. Monorepo with `backend/` (REST API) and `frontend/` (SPA), each its own pnpm package.
...
- List all donations, "my donations", detail, update, delete.
...
frontend/
  ...
  src/features/{auth,adoption,children,dashboard,donations,landing,reports,volunteers}/
...
- `User` — role, email verification state, relations to reports/donations/adoption requests.

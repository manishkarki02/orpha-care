...
export const getMyDonations = async (
	query: GetDonationsRequestSchema["query"],
	donorId: string,
) => {
...
export const getDonationById = async (donationId: string, user: { id: string; role: Role }) => {
...
export const updateDonationStatus = async (
	...
	return await prisma.$transaction(async (tx) => {
		...
		if (body.status === DonationStatus.Pending) {
			throw new BadRequestError("Donation status cannot be changed back to pending.");
		}

...
describe("auth.service", () => {
	...
	describe("signUpUser", () => {
		it("throws BadRequestError when email/phone is already registered and verified", async () => {
			...
			await expect(authService.signUpUser(registerBody)).rejects.toBeInstanceOf(BadRequestError);
		});
	...
	describe("signInUser", () => {
		it("throws AuthenticationError for an unknown email", async () => {
			...
			await expect(authService.signInUser(loginBody)).rejects.toBeInstanceOf(AuthenticationError);
		});
